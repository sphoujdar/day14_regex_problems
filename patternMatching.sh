...
declare -a passwords_check=("1236E547989" "duaw#dvgADws1" "----------------------------------------------" "shubham phoujdar" "dsawdg##ht" "184556" "adwdf")
...
echo ------------------PASSWORD CHECK-----------------------
        for password in "${passwords_check[@]}" ; do
                if [[ $password =~ $password_pattern ]] ; then
                        echo \[ $password \] accepted
                else echo \[ $password \] rejected
                fi
        done
